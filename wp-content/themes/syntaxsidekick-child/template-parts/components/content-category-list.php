<?php
/**
 * Shared content category list card.
 *
 * @package SyntaxSidekick_Child
 */

$section_id = isset($args['id']) ? (string) $args['id'] : 'ss-browse-content-title';
$section_heading = isset($args['heading']) ? (string) $args['heading'] : 'Browse Content';
...
<section class="ss-sidebar-card" aria-labelledby="<?php echo esc_attr($section_id); ?>">
    ...
    <ul class="ss-sidebar-list">
        <?php foreach ($items as $item) : ?>
            ...
            $icon_name = function_exists('syntaxsidekick_get_topic_icon') ? syntaxsidekick_get_topic_icon($slug) : '';
            ...
            $icon_svg = $icon_name && function_exists('syntaxsidekick_icon')
                ...
                : '';
            $fallback_text = strtoupper(substr('all' === $slug ? 'AC' : $label, 0, 'all' === $slug ? 2 : 1));
            ?>
            <li>
